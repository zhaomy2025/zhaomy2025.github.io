...
加密算法的安全性直接影响数据的机密性、完整性和可用性。随着计算能力的提升和攻击技术的发展，算法本身、算法实现以及算法在协议中的使用方式都可能成为攻击入口。下面按对称加密、非对称加密、哈希函数三类，整理常见的攻击方式与对应的防御手段。
...
工程实践：核心数据采用AES-256-GCM加密，密钥轮换机制每90天生成新密钥并重新加密数据。
...
防御策略：
- 实现**恒定时间**算法，避免条件分支和内存访问依赖密钥
- 使用屏蔽技术（如电磁屏蔽、电源滤波）
- 加入随机化因素（如随机延迟、随机化密钥调度）
- 采用安全的硬件实现（如防篡改芯片）

工程实践：加密操作放在安全元件（SE）中执行，该元件具有防侧信道攻击的硬件设计，可抵御功耗分析和电磁辐射攻击。
...
工程实践：混合密钥对（RSA-4096 + CRYSTALS-Kyber）可以在保持向后兼容的前提下引入后量子算法。

### 离散对数攻击（针对ECC和Diffie-Hellman）
...
案例分析：Curve25519是目前广泛推荐的椭圆曲线，已被TLS 1.3等现代协议采用。
...
工程实践：客户端内置服务器证书的公钥哈希（证书钉扎），只与预期的服务器通信。
...
工程实践：代码仓库禁用MD5和SHA-1，新的哈希计算统一使用SHA-256或SHA-3算法。
...
案例分析：bcrypt算法通过引入工作因子（cost factor）来增加哈希计算的时间复杂度，抵御彩虹表攻击和暴力破解攻击。
...
工程实践：定期对在用的加密算法和实现做渗透测试，模拟暴力破解、侧信道等攻击场景。

### 同行评审

方法：通过学术界和工业界的广泛审查，发现潜在的安全问题。

优势：参与评审的人多、背景分散，能覆盖单一团队容易忽略的问题。

案例：AES算法经过全球密码学家3年多的评审，最终成为国际标准。
...
- 选择安全的模式：优先选择**认证加密**模式（如GCM、CCM）
- 避免使用默认参数：根据应用场景调整参数（如哈希函数的工作因子）

工程实践：加密参数集中管理，各服务使用同一套安全参数配置。
...
上述攻击可以按作用对象分成三类：差分攻击、线性攻击和碰撞攻击针对算法设计，大数分解和离散对数攻击针对数学难题，侧信道和计时攻击针对实现。前两类靠选择经过充分分析的算法和足够大的参数规模规避，第三类只能在实现层面处理。

当前可用的参数基线是：对称加密用AES-256或ChaCha20-Poly1305，非对称加密用RSA（至少2048位，长期安全用4096位）或ECC Curve25519，哈希函数用SHA-256或SHA-3-256，密码存储用Argon2id或bcrypt。DES的S盒设计缺陷、MD5和SHA-1的碰撞攻击都说明，算法本身被攻破后，加长密钥和加固实现都无法补救，只能替换算法。

量子计算是唯一会推翻上述基线的变量。Shor算法能在多项式时间内解决大整数分解和离散对数问题，RSA和ECC都需要迁移到后量子算法；可行路径是混合密钥对（传统算法加后量子算法）并行运行，在保持向后兼容的同时引入后量子能力。
